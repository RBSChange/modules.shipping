jQuery(document).ready(function() {
	...
	for (var i = 0; i < markers.length; i++)
	{
		...
		var markerOptions = { ref: markerdata.ref, index: i};
		if ('iconUrl' in markerdata)
		{
			markerOptions.icon = markerdata.iconUrl;
		}
		var marker = gmaps_addMarkerToMap(map, markerdata.latitude, markerdata.longitude, markerdata.name, markerOptions);
...
function relay_attachMarkerEvent(marker)
{
	...
	google.maps.event.addListener(marker, 'mouseover', function () 
		{ 
			...
			jQuery('#relay-over-infos img').css('max-width', jQuery('#relay-over-infos').width() + 'px');
			jQuery('#relay-over-infos .columns').removeClass('columns').addClass('normal-margin');
			jQuery('#relay-over-infos .column-50').removeClass('column-50').addClass('normal-margin');
		});
}
